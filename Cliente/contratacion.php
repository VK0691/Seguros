...
Cuota a pagar del seguro: \${$monto_seguro}
Estado civil: {$estado_civil}
Cónyuge: {$nombre_conyuge} ({$cedula_conyuge})
Dependientes: {$datos_hijos}
        ...
        <form method="POST" action="" enctype="multipart/form-data" class="needs-validation" id="form-firma" novalidate>
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="file" name="cedula_escan[]" class="form-control" accept=".pdf,.jpg,.png,.jpeg,.docx" multiple>
                    <small class="text-muted">PDF, PNG, JPG, JPEG, DOCX. Max. 3</small>
                </div>
                <div class="col-md-6">
                    <label>Sexo</label>
                ...
                <div class="col-md-6" id="campo_conyugue_cedula" style="display:none">
                    <label>Nro. de Identificación del cónyuge</label>
...
function validarFormulario() {
    ...
    return true; // Permite el envío del formulario
                ...
                <div class="col-12" id="campo_num_hijos">
                    ...
                    <input type="number" id="num_hijos" name="num_hijos" min="0" max="10" class="form-control" value="">
                    <div id="advertencia_hijos" class="text-danger mt-1" style="display:none">No es aplicable para el seguro</div>
                    <small class="text-muted">Ponga 0 si no aplica</small>
                </div>

                <div class="col-12" id="datos_hijos"></div>
                <div class="col-md-6">
                    <label>Subir documentos adicionales</label>
                    <input type="file" name="documentos_adicionales[]" class="form-control" accept=".pdf,.jpg,.png,.jpeg,.docx" multiple>
...
<div class="col-12 mt-4">
    ...
    <a href="clientedash.php" class="btn btn-secondary">
...
document.getElementById('num_hijos').addEventListener('input', function() {
    const numHijos = parseInt(this.value);
    const datosHijosDiv = document.getElementById('datos_hijos');
    const advertencia = document.getElementById('advertencia_hijos');

    datosHijosDiv.innerHTML = ''; // Limpiar contenido anterior
    advertencia.style.display = 'none'; // Ocultar mensaje al principio

    if (isNaN(numHijos) || numHijos <= 0) {
        return; // No hacer nada si está vacío o menor/igual a cero
    }
...
document.addEventListener('DOMContentLoaded', function() {
    // Límite para documentos escaneados
    const cedulaInput = document.querySelector('input[name="cedula_escan[]"]');
    cedulaInput.addEventListener('change', function() {
        if (this.files.length > 3) {
            alert('Solo puedes subir un máximo de 3 archivos para documentos escaneados.');
            this.value = ''; // Limpiar selección
        }
    });

    // Límite para documentos adicionales
    const adicionalesInput = document.querySelector('input[name="documentos_adicionales[]"]');
    adicionalesInput.addEventListener('change', function() {
        if (this.files.length > 10) {
            alert('Solo puedes subir un máximo de 10 archivos para documentos adicionales.');
            this.value = ''; // Limpiar selección
        }
    });
});
...
document.getElementById('form-firma').addEventListener('submit', function (e) {
    ...
    // Quita bordes rojos anteriores
    form.querySelectorAll('.is-invalid').forEach(el => {
        el.classList.remove('is-invalid');
    });

    // Revisa solo los campos que están visibles
    const campos = form.querySelectorAll('input, select, textarea');
    campos.forEach(campo => {
        const visible = campo.offsetParent !== null;

        if (visible && campo.hasAttribute('required') && !campo.value.trim()) {
    ...
    if (camposVacios) {
        ...
        mostrarAlertaToast("Ningún campo puede estar vacío.");
    }
});
