...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['solicitud_id']) && isset($_POST['accion'])) {
    ...
    try {
        ...
        // Obtener el ID de usuario para la notificación
        $sql_usuario_id = "SELECT usuario_id FROM seguros_vida WHERE id = ?";
        $stmt_usuario_id = $conn->prepare($sql_usuario_id);
        $stmt_usuario_id->bind_param("i", $id);
        $stmt_usuario_id->execute();
        $result_usuario_id = $stmt_usuario_id->get_result();
        $row_usuario = $result_usuario_id->fetch_assoc();
        ...
        if ($accion === 'aprobar' && !empty($_POST['monto_asegurado'])) {
            ...
            $mensaje_noti = "Tu solicitud fue aprobada. Póliza: $numero_poliza.";
            $tipo = "aprobacion";
            $tipo_referencia = "seguro";
            $accion_url = "ver_solicitud.php?id=$id";
            ...
            $mensaje_noti = "Tu solicitud fue rechazada. Puedes revisar los detalles.";
            $tipo = "rechazo";
            $tipo_referencia = "seguro";
            $accion_url = "ver_solicitud.php?id=$id";
            $accion_texto = "Ver detalles";
        ...
        if ($stmt_update->execute()) {
            ...
            $stmt_noti = $conn->prepare("CALL crear_notificacion(?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt_noti->bind_param("issssiss", 
                $usuario_id, 
                $titulo, 
                $mensaje_noti, 
                $tipo, 
